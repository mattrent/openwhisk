...
//                      propagateInvocation(action, msg, transid, policyIndex, actionTag, None)
  ...
  def propagateInvocation(action: ExecutableWhiskActionMetaData,
                          msg: ActivationMessage,
                          transid: TransactionId,
                          policyIndex: Int,
                          actionTag: String,
                          availableIndexes: Option[List[Int]]): Option[(InvokerInstanceId, Boolean)] = {
    val configurableLBSettings: ConfigurableLBSettings = schedulingState.configurableLBSettings
...
//                        Await.result(future, timeout.duration).asInstanceOf[Option[(InvokerInstanceId, Boolean)]]
  ...
  def schedulePropagated(action: ExecutableWhiskActionMetaData,
                         msg: ActivationMessage,
                         transid: TransactionId,
                         policyIndex: Int,
                         actionTag: String,
                         availableIndexes: List[Int]): Option[(InvokerInstanceId, Boolean)] = {
    val isBlackboxInvocation = action.exec.pull
    val (invokersToUse, stepSizes) =
      if (!isBlackboxInvocation) (schedulingState.managedInvokers, schedulingState.managedStepSizes)
      else (schedulingState.blackboxInvokers, schedulingState.blackboxStepSizes)
    ...
    chosenInvoker
    //.orElse(propagateInvocation(action, msg, transid, policyIndex, actionTag, Some(availableIndexes)))
  }
...
object ConfigurableLoadBalancer extends LoadBalancerProvider {
  ...
  override def instance(whiskConfig: WhiskConfig, instance: ControllerInstanceId)(
    ...
    new ConfigurableLoadBalancer(whiskConfig, instance, createFeedFactory(whiskConfig, instance), invokerPoolFactory)
  }
  ...
  def modifiedDefaultSchedule(
    ...
    if (numInvokers > 0) {
      ...
      val invokerInvalidate = invalidateMap
        .get(invoker.id.uniqueName.getOrElse(""))
        .getOrElse(
          maxCap.getOrElse(action.limits.memory.megabytes),
          maxCon.getOrElse(action.limits.concurrency.maxConcurrent))
      if (invoker.status.isUsable && dispatched(invoker.id.toInt)
            .tryAcquireConcurrent(fqn, invokerInvalidate._2, invokerInvalidate._1)) {
        ...
        if (stepsDone == numInvokers + 1) {
          ...
          if (healthyInvokers.nonEmpty) {
            ...
            val invokerInvalidate = invalidateMap
              .get(random.id.uniqueName.getOrElse(""))
              .getOrElse(
                maxCap.getOrElse(action.limits.memory.megabytes),
                maxCon.getOrElse(action.limits.concurrency.maxConcurrent))
  ...
  def defaultSchedule(
    ...
    invalidateMap: Map[String, (Int, Int)],
    maxCap: Option[Int],
    maxCon: Option[Int])(implicit logging: Logging, transId: TransactionId): Option[(InvokerInstanceId, Boolean)] = {
  ...
  def filterInvokers(invokerList: IndexedSeq[InvokerHealth],
                     label: Option[String],
                     labelMap: Map[String, String]): IndexedSeq[InvokerHealth] = {
    invokerList.filter(i =>
  ...
  def schedule(
    ...
    policyIndex: Int)(implicit logging: Logging, transId: TransactionId): Option[(InvokerInstanceId, Boolean)] = {
    ...
    def bestFirstSchedule(specifiedInvokers: List[InvokerHealth],
      ...
      specifiedInvokers match {
        ...
        case (x: InvokerHealth) :: xs =>
          val invokerInvalidate = invalidateMap
            .get(x.id.uniqueName.getOrElse(""))
            .getOrElse(maxCap.getOrElse(slots), maxCon.getOrElse(maxConcurrent))
          if (dispatched(x.id.toInt).tryAcquireConcurrent(fqn, invokerInvalidate._2, invokerInvalidate._1))
            Some(x.id, false)
          else
      ...
      specifiedInvokers match {
        ...
        case (_: InvokerHealth) +: _ =>
          ...
          if (dispatched(x.id.toInt).tryAcquireConcurrent(fqn, invokerInvalidate._2, invokerInvalidate._1))
            ...
            randomSchedule(
              specifiedInvokers.take(rIndex - 1) ++ specifiedInvokers.drop(rIndex),
              invalidateMap,
              maxCap,
              maxCon)
      }
    ...
    def scheduleOnInvokerLabels(healthyInvokers: IndexedSeq[InvokerHealth],
                                sameZoneInvokers: IndexedSeq[InvokerHealth],
                                sets: List[WorkerSet],
      ...
      sets match {
        ...
        case (l: WorkerSet) :: ls =>
          ...
          val chosenInvoker = scheduleByStrategy(
            cInvokers,
          ...
          if (strategy == "best-first") {
            ...
            chosenInvoker.orElse(
              scheduleOnInvokerLabels(
                healthyInvokers,
                sameZoneInvokers,
                sets.take(currentIndex - 1) ++ sets.drop(currentIndex),
                blockStrategy))
          }
      }
    }

    def scheduleOnSpecifiedBlock(healthyInvokers: IndexedSeq[InvokerHealth],
                                 sameZoneInvokers: IndexedSeq[InvokerHealth],
      ...
      blockSettings.workersSettings match {
        case WorkerList(workers) =>
          ...
          //empty list for sameZoneInvokers, as we do not prioritize workers in the same topological zone when a list is explicitly specified
            ...
            val invalidateMap = workers
              .map(w => {
                val maxCap = w.maxCapacity.orElse(blockSettings.maxCapacity).getOrElse(slots)
                val maxCon =
                  w.maxConcurrentInvocations.orElse(blockSettings.maxConcurrentInvocations).getOrElse(maxConcurrent)
...
case class ConfigurableLoadBalancerState(
  ...
  lbConfig: ShardingContainerPoolBalancerConfig = loadConfigOrThrow[ShardingContainerPoolBalancerConfig](
    ConfigKeys.loadbalancer))(implicit logging: Logging, controllerInstance: ControllerInstanceId) {
  ...
  def refreshNodeZoneMap(): Map[String, String] = {
    val associations = try {
      scala.io.Source
        .fromFile("/data/node_zone_map.json")
        .getLines
        .mkString("\n")
        .parseJson
  ...
  def refreshNodePodMap(): Map[String, String] = {
    val associations = try {
      scala.io.Source
        .fromFile("/data/node_pod_map.json")
        .getLines
        .mkString("\n")
        .parseJson
        .convertTo[Map[String, String]]
  ...
  def refreshNodeLabelMap(): Map[String, String] = {
    val associations = try {
      scala.io.Source
        .fromFile("/data/node_label_map.json")
        .getLines
        .mkString("\n")
        .parseJson
        .convertTo[Map[String, String]]
   ...
   */
  private def getInvokerSlot(memory: ByteSize,
                             invokerName: Option[String],
                             controllerZone: String,
                             sameZoneControllersCount: Int,
                             nodeZoneMap: Map[String, String],
                             nodePodMap: Map[String, String],
    ...
    val newTreshold =
      if (invokerShardMemorySize < MemoryLimit.MIN_MEMORY && invokerDistribution != "isolated" && (invokerZone == controllerZone || noZone)) {
        logging.error(
          this,
        ...
        logging.info(
          this,
          s"Cluster size ${_clusterSize}, invoker user memory size ${memory.toMB.MB}, "
            + s"min action memory size ${MemoryLimit.MIN_MEMORY.toMB.MB}, calculated shard size ${invokerShardMemorySize.toMB.MB} for Invoker ${invokerName}.")(
          TransactionId.loadbalancer)
  ...
  def updateInvokers(newInvokers: IndexedSeq[InvokerHealth]): Unit = {
      ...
      if (oldSize < newSize) {
        ...
        _invokerSlots = _invokerSlots ++ onlyNewInvokers.map { invoker =>
          new NestedSemaphore[FullyQualifiedEntityName](
            getInvokerSlot(
              invoker.id.userMemory,
              invoker.id.uniqueName,
              controllerZone,
              sameZoneControllersCount,
              nodeZoneMap,
              nodePodMap,
              distribution).toMB.toInt)
        }
    ...
    logging.info(
      ...
      TransactionId.loadbalancer)
    logging.info(this, s"Cluster size ${_clusterSize}, InvokerSlots: ${_invokerSlots}, Invokers: ${_invokers}")(
      TransactionId.loadbalancer)
  }
 ...
 *
 */
case class PropagateInvocation(action: JsValue,
                               msg: JsValue,
                               transid: JsValue,
                               policyIndex: Int,
                               actionTag: String,
                               availableIndexes: List[Int],
